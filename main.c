...
//                                           CREATED BY BMG
// History:
// 17.01.2024 Initial release for 115200 baud at 16 MHz
// 02.02.2024 Added threshold routing and dual DAC support on PB4
// 06.02.2024 Refined UART Tx/Rx and adc_single_measure()
// 02.10.2024 Rev 3.1 update: additional addresses, ADC remap, PTC refinements
// 19.09.2025 Applied daclsb mask (0xfc) in write command handling
// 09.12.2025 Reduced spam for 'i' command, added ACK in palka_set, fixed command parsing for '&'
//-----------------------------------------------------------------------------------------------------
...
#define I2C_TIMEOUT 1000  // Timeout used to avoid I2C lockups
#define ADC_TIMEOUT 100000  // Timeout used to avoid ADC conversions hanging
...
int main()
{
  ...
  sadelay(); // delay based on address to stagger startup
  sendMessage(" %c ready \r\n",oa);
  cmdl[0] = 0;
...
void EEPROM_Write(uint16_t __eeprom *address, uint16_t value) {
    ...
    *address = value; // Write 16-bit value to EEPROM
...
void SetDacAfterRestart() {
    ...
    for (uint8_t dac_channel = 0; dac_channel < 16; dac_channel++) {
        if (eeV[dac_channel] != 0xFFFF) {  // Skip empty values
            msb = (uint8_t)(eeV[dac_channel] >> 8);  // High byte
            lsb = (uint8_t)(eeV[dac_channel] & 0xFF); // Low byte

            // Restore previous output levels
...
void resetI2C() {
  I2C1_CR2_bit.STOP = 1;            // Force STOP to clear the bus
  I2C1_CR1_bit.PE = 0;             // Disable I2C peripheral
  for (volatile int i = 0; i < 1000; i++); // Small delay for bus recovery
  I2C1_CR1_bit.PE = 1;             // Re-enable I2C peripheral
}
...
int MyLowLevelPutchar(int sendchar)
{
  ...
  while(!USART1_SR_TC && !USART1_SR_TXE);  // Wait until transmission complete
...
unsigned int adc_single_measure(uint8_t channel)
{
  ...
  uint32_t adc_timeout = ADC_TIMEOUT;

  ADC1_CR1_bit.START = 1;

  while (!(ADC1_SR & MASK_ADC1_SR_EOC) && adc_timeout--)
  {
  };
...
unsigned int adc_multiple_measure(uint8_t channel) {
  ...
  for (int i = 0; i < 16; i++) { // Average 16 samples
    ...
    unsigned int adc_res = (ADC1_DRH << 8) | ADC1_DRL; // Read conversion result
    sum += adc_res; // Accumulate samples
  }

  average = sum / 16; // Average samples
  ...
  return average; // Return averaged result
}
...
#pragma vector = USART1_R_OR_vector          // USART1 receive interrupt vector
